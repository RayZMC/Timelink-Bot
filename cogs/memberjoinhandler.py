...
class MemberJoinHandler(commands.Cog):
    ...
    async def on_member_join(self, member: discord.Member):
        ...
        bg.ellipse((835, 340), 250, 250, outline="black", stroke_width=5)

        bg.text((960, 620), f"Welcome to {member.guild.name}!", color="black", font=font_big, align="center")
        bg.text((960, 740), f"{member.name} is member #{member.guild.member_count}!", color="black", font=font_small, align="center")

        img_file = discord.File(fp=bg.image_bytes, filename=randomized_image)

        await welcome_channel.send(f"Hello there, {member.name}! Be sure to read our rules and follow them carefully. Thank you for joining")
